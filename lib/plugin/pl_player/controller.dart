...
class PlPlayerController with BlockConfigMixin {
  ...
  void enterPip({bool autoEnter = false}) {
    if (hasVideoView) {
      final state = _androidHdrBackend?.state;
      final stateWidth = state?.width ?? videoPlayerController?.state.width ?? 0;
      final stateHeight =
    ...
    _backendSubscription = backend.events.listen((event) {
      ...
      if (event.status case final status?) {
        ...
        if (status == PlayerStatus.completed) {
          ...
          makeHeartBeat(position.value, type: .status);
        }
      }
  ...
  void onForward(Duration duration) {
    onForwardBackward(Duration(seconds: position.value) + duration);
  }
  ...
  Future<void> takeScreenshot() async {
    ...
    Uint8List? bytes;
    if (_androidHdrBackend case final backend?) {
      bytes = await backend.screenshot();
    } else if (videoPlayerController case final player?) {
